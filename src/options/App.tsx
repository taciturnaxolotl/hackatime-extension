...
const TokenForm = () => {
	...
	return (
		<>
			<form
				method="post"
				className="flex flex-row self-center w-[60%]"
			...
			>
				<div className="relative w-full">
					...
					<button
						type="button"
						className={`absolute right-1 top-1/2 transform -translate-y-1/2 scale-125 transition-transform duration-75 ${hideAnimate ? (showToken ? "!scale-110" : "scale-135") : ""}`}
						onClick={() => {
							setShowToken(!showToken);
							setHideAnimate(true);
							setTimeout(() => setHideAnimate(false), 100);
						}}
					>
				...
				<button
					type="submit"
					className={`btn-primary m-2 transition-transform duration-150 ml-5 ${animate ? "button-pop" : ""}`}
				>
					Submit
				</button>
			</form>
			<p
				className={`text-text-secondary dark:text-text-dark-secondary italic text-sm transition-transform duration-200 ${animate ? "animate-pop" : ""}`}
			>
				{message}
			</p>
